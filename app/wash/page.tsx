...
const sanitationData = [
  { name: "VIP Latrine", count: 450, fill: "#10B981" },      
  { name: "Traditional Pit", count: 1200, fill: "#F59E0B" }, 
  { name: "Shared/Communal", count: 300, fill: "#6366F1" },  
  { name: "Open Defecation", count: 150, fill: "#EF4444" },  
];
    ...
    <main className="space-y-6 md:space-y-8 bg-gray-50/50 min-h-screen p-4 sm:p-6">
      ...
      <header className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 border-b border-gray-200 pb-4 md:pb-6">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">Infrastructure Monitor</h1>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col">
          ...
          <div className="flex-1 min-h-[200px] sm:min-h-[250px] relative">
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center -mt-5 pointer-events-none">
              <span className="text-2xl sm:text-3xl font-extrabold text-gray-900">65%</span>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col">
          ...
          <div className="flex-1 min-h-[200px] sm:min-h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sanitationData} layout="vertical" margin={{ left: 80, right: 10 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#F3F4F6" />
                <XAxis type="number" hide />
                <YAxis dataKey="name" type="category" width={80} tick={{fontSize: 11, fontWeight: 500, fill: '#374151'}} axisLine={false} tickLine={false} />
                <RechartsTooltip cursor={{fill: '#F9FAFB'}} />
                <Bar dataKey="count" radius={[0, 4, 4, 0]} barSize={20} background={{ fill: '#F9FAFB' }} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <section className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h3 className="font-bold text-gray-900 flex items-center gap-2">
          ...
          <button className="text-xs font-bold text-blue-600 flex items-center gap-1 hover:underline w-full sm:w-auto">
            View Maintenance Log <ArrowRight size={12} />
          </button>
        </div>
        <div className="divide-y divide-gray-100">
          {brokenInfrastructure.map((item) => (
            <div key={item.id} className="p-4 md:px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between hover:bg-gray-50 transition-colors gap-4">
